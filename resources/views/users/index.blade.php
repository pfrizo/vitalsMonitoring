<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div x-data="{ confirmingUserDeletion: false, userToDelete: null }" class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <table class="w-full table-fixed divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="w-1/3 px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Email</th>
                                    <th class="w-1/6 px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Função</th>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                            <div class="flex justify-center space-x-4">
                                                ...
                                                @if(auth()->id() !== $user->id)
                                                    <button @click.prevent="userToDelete = {{ $user->id }}; confirmingUserDeletion = true" 
                ...
                <div x-show="confirmingUserDeletion" 
                    ...
                    <div x-show="confirmingUserDeletion" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="confirmingUserDeletion = false"></div>
                    ...
                    <div x-show="confirmingUserDeletion" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative bg-white rounded-lg shadow-xl overflow-hidden transform transition-all sm:my-8 sm:max-w-lg sm:w-full">
                        ...
                        <form method="POST" :action="`/users/${userToDelete}`">
                            @csrf
                            @method('DELETE')
                                    ...
                                    <div class="mt-3 text-center sm:mt-0 sm:ms-4 sm:text-left">
                                        ...
                                        <div class="mt-2">
                                            <p class="text-sm text-gray-500">
                                                Você tem certeza que deseja excluir este usuário? O acesso dele será revogado imediatamente.
                                            </p>
                                        </div>
                                    </div>
                                </div>
